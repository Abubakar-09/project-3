...
const Products = (props) => {
    const [numberOfProducts, setNumberOfProducts] = useState(parseInt(props.length))
    const [typeOfProducts, setTypeOfProducts] = useState(props.type || '')
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                ...
                // Check if the type is 'All', if true, take all products
                if (typeOfProducts === "All") {
                    filteredProducts = data;
                } else {
                    filteredProducts = data.filter(item => item.type === typeOfProducts);
                }

                // Slice to ensure we only get the desired number of products
                setProducts(filteredProducts.slice(0, numberOfProducts));
            } catch (error) {
        ...
        getData();
    }, [numberOfProducts, typeOfProducts, data]);
                ...
                {products ? (products.map((e, index) => {
                    return <div key={index}>
                        <div className='border-[1px] relative border-black md:hover:scale-105 flex items-center md:w-[18vw] rounded-lg max-md:h-[65vh] h-[55vh] text-black text-sm'>
                            ...
                            <div onClick={() => { handleWishlist(e) }} className="absolute p-2 top-1 right-1 hover:scale-125 rounded-full border-[1px] border-black">
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="20"
                                    height="20"
                                    viewBox="0 0 24 24"
                                    fill={`${e.wishlist == true ? "red" : "none"}`}
                                    stroke="black"
                                    stroke-width="1.25"
